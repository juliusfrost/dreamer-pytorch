...
def build_and_train(log_dir, name="MineRLTreechop-v0", run_ID=0, cuda_idx=None, eval=False, save_model='last',
    ...
    def make_env(name, frame_skip=2):
        env = gym.make(name)
        env = wrap_env(env, name, frame_skip=frame_skip)
        return env

    factory_method = make_wapper(
        make_env,
        [GymEnvWrapper, OneHotAction],
        [dict(), dict()])
    sampler = SerialSampler(
        ...
        # eval_n_envs=10,  # multiple environments don't work on minerl https://github.com/minerllabs/minerl/issues/210
        # eval_max_steps=int(10e3),
        # eval_max_trajectories=5,
    )
    algo = Dreamer(horizon=10, kl_scale=0.1, use_pcont=True, initial_optim_state_dict=optimizer_state_dict,
                   replay_size=int(1e4))  # Debugging
    agent = DiscreteDreamerAgent(train_noise=0.4, eval_noise=0, expl_type="epsilon_greedy",
...
if __name__ == "__main__":
    ...
    parser.add_argument('--name', help='Minecraft task name', default='MineRLTreechop-v0', choices=ENVIRONMENTS)
